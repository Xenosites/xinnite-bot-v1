...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/91ce736e32b8ccae3046c.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Ayptrii', body: wm, sourceUrl: 'https://instagram.com/rijalsavior', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
